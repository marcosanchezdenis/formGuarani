 ...
 */
    public function __construct(View $View, $settings = array()) {
        ...
        function render_label($settings=array()) {
    ...
    function render_input($settings=array(),$class) {//implementacion rapida de class para agregar el estilo
        ...
        switch( $this->settings['type']):
            case 'image':
                ...
                //echo $this->Html->image($this->Html->url(array('controller'=>$controller, 'action'=>$action, '?'=> $qstring), true), array('vspace'=>2));
                //echo $this->Html->link( $this->settings['reload_txt'], '#', array('class' => 'creload', 'escape' => false));
                echo $this->Form->input( $this->settings['model'].'.'. $this->settings['field'], array('autocomplete'=>'off','label'=> $this->settings['clabel'],'class'=>$class));
            break;
            case 'math':
                $qstring = array_merge($qstring, array('type'=>'math'));
                if(isset($this->settings['stringOperation']))   {
                ...
                echo $this->Form->input( $this->settings['model'].'.'. $this->settings['field'],array('autocomplete'=>'off','label'=>false,'class'=>$class));
            break;
        endswitch;
    }
